...
export function PortGrid({ ports }: { ports: Port[] }) {
      ...
      <div className="grid grid-cols-3 gap-3">
        {items.map((port) => (
          <div key={port.id} className={cn('rounded-xl border p-3 text-sm', port.status === 'busy' ? 'border-rose-200 bg-rose-50' : 'border-emerald-200 bg-emerald-50')}>
            <p className="font-semibold text-slate-900">Port {port.port_number}</p>
            <p className="mt-1 text-xs text-slate-600">{port.cable_type}</p>
            <p className="mt-2 text-xs font-medium uppercase tracking-[0.2em] text-slate-500">{port.status}</p>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      {renderSection('Patch ports', patchPorts, 'patch')}
      {renderSection('Power ports', powerPorts, 'power')}
    </div>
  );
}
